...
export let StoreManager = class StoreManager {
  constructor(grid) {
  ...
  _subscribeToDataCollectionChanges(grid) {
    this.dataCollectionSubscription = grid.bindingEngine.collectionObserver(grid.data).subscribe(collectionChangeInfo => {
      grid.refresh();
    });
  }
  ...
  _getDataStore(grid) {
    ...
    if (grid.data !== null) {
      ...
      this.dataPropertySubscription = grid.bindingEngine.propertyObserver(grid, 'data').subscribe((newItems, oldItems) => {
