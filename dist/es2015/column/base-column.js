export let BaseColumn = class BaseColumn {
  ...
  _updateViewModelOnPropertyChange(viewModel, viewModelPropertyName, columnPropertyName) {
    ...
    if (viewModelPropertyNameTokens.length > 1) {
      subscription = this.grid.bindingEngine.propertyObserver(this, columnPropertyName).subscribe((newValue, oldValue) => {
        let obj = viewModel;
        let i;
  ...
  subscribe(viewModelPropertyName, columnPropertyName) {
    ...
    if (viewModelPropertyNameTokens.length > 1) {
      ...
      subscription = this.grid.bindingEngine.expressionObserver(viewModel, viewModelPropertyName).subscribe((newValue, oldValue) => {
        this.setColumnProperty(columnPropertyName, newValue);
      });
    } else {
      value = viewModel[viewModelPropertyName];
      subscription = this.grid.bindingEngine.propertyObserver(viewModel, viewModelPropertyName).subscribe((newValue, oldValue) => {
        this.setColumnProperty(columnPropertyName, newValue);
      });
    }
